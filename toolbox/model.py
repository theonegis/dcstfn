...
def dcstfnex(coarse_input, fine_input, coarse_pred, d=[32, 64, 128]):
    pool_size = 2
    coarse_model = _htls_net(coarse_input, coarse_pred, d)
    fine_model = _hslt_net(fine_input, d)
...
def dcstfn(coarse_input, fine_input, coarse_pred, d=[32, 64, 128]):
    ...
    merged_layer = Conv2DTranspose(d[1], 3, strides=pool_size,
                                   padding='same',
                                   kernel_initializer='he_normal')(added_layer)
    final_out = Dense(fine_input.shape[-1])(merged_layer)
    model = Model([coarse_input_layer, fine_input_layer, coarse_pred_layer], final_out)
    return model


def _hslt_net(fine_input, d, pool_size=2):
    # 对于Landsat高分辨率影像建立网络
...
def _htls_net(coarse_input, coarse_pred, d):
    # 对于两张MODIS影像建立相同的网络
